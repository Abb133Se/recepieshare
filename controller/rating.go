...
func PostRatingHandler(c *gin.Context) {
	...
	if rating.Score < 1 || rating.Score > 5 {
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: "score out of bounds, it must be between 1 and 5"})
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, ErrorResponse{Error: "failed to fetch user data"})
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, ErrorResponse{Error: "failed to fetch recipe data"})
		return
	}
	...
	if err == nil {
		...
		c.JSON(http.StatusInternalServerError, ErrorResponse{Error: "failed to check existing rating"})
		return
	}
